...
function addOrder() {
    ...
    if (parseFloat(qty1.value) > 0) {
        var order = `${qty1.value} pc/s x ${price1.textContent} ------ Eyeshadow w/ 9pcs Makeup Set ------ Php ${(parseFloat(qty1.value) * parseFloat(price1.textContent)).toFixed(2)}\n`;
        carts.textContent += order;
        totalAmount += parseFloat(qty1.value) * parseFloat(price1.textContent);
    }

    if (parseFloat(qty2.value) > 0) {
        var order = `${qty2.value} pc/s x ${price2.textContent} ------ Eyeshadow w/ 8pcs Makeup Set ------ Php ${(parseFloat(qty2.value) * parseFloat(price2.textContent)).toFixed(2)}\n`;
        carts.textContent += order;
        totalAmount += parseFloat(qty2.value) * parseFloat(price2.textContent);
    }

    if (parseFloat(qty3.value) > 0) {
        var order = `${qty3.value} pc/s x ${price3.textContent} ------ Setting Spray w/ Foundation Concealer ------ Php ${(parseFloat(qty3.value) * parseFloat(price3.textContent)).toFixed(2)}\n`;
        carts.textContent += order;
        totalAmount += parseFloat(qty3.value) * parseFloat(price3.textContent);
    }

    if (parseFloat(qty4.value) > 0) {
        var order = `${qty4.value} pc/s x ${price4.textContent} ------ Eyeliner, Eyebrow, Mascara with Curler ------ Php ${(parseFloat(qty4.value) * parseFloat(price4.textContent)).toFixed(2)}\n`;
        carts.textContent += order;
        totalAmount += parseFloat(qty4.value) * parseFloat(price4.textContent);
    }

    if (parseFloat(qty5.value) > 0) {
        var order = `${qty5.value} pc/s x ${price5.textContent} ------ Multi-use Duo Makeup Brushes ------ Php ${(parseFloat(qty5.value) * parseFloat(price5.textContent)).toFixed(2)}\n`;
        carts.textContent += order;
        totalAmount += parseFloat(qty5.value) * parseFloat(price5.textContent);
    }

    if (parseFloat(qty6.value) > 0) {
        var order = `${qty6.value} pc/s x ${price6.textContent} ------ Eyebrow wax and Eyeliner Stamp ------ Php ${(parseFloat(qty6.value) * parseFloat(price6.textContent)).toFixed(2)}\n`;
        carts.textContent += order;
